function plot_ranges(r_op_ranges, theta_op_ranges)
    % --- Loop through all ranges ---
    for i = 1:size(theta_op_ranges, 1)
        for j = 1:size(r_op_ranges, 1)
            theta_lo = theta_op_ranges(i, 1);
            theta_hi = theta_op_ranges(i, 2);
            r_lo = r_op_ranges(j, 1);
            r_hi = r_op_ranges(j, 2);
